...
from .transforms import Zoom, ArrayToTensor

from .flow_utils import flow_to_image, calc_corner_bbox_freq, \
...
class TestHelper:
    ...
    def inference_flows(self, image_list, gap=3, init_adjacent=4):
        ...
        flows = self.run_sequence(imgs, size=(h, w), gap=gap, init_adjacent=init_adjacent)
        return flows
...
def init_module(model=None, test_shape=None):
    if test_shape is None:
        test_shape = [384, 640]
    if model is None:
        model = os.path.join("~/mmaction2/checkpoints", "pwclite_ar_mv.tar")
