...
from ...utils import get_random_string, get_shm_dir, get_thread_id, imdecode
...
class NoriDecode:
    ...
    def __init__(self, io_backend='disk', decoding_backend='cv2', **kwargs):
        ...
        self.kwargs = kwargs
        self.file_client = None
        self.im_keys = ["nori_id_seq", "nids_flow_img"]
        self.flow_keys = ["nids_flow"]
        self.box_keys = ["gt_bboxes", "prposals"]
        self.key_map = dict(nori_id_seq="imgs", nids_flow_img="flow_imgs",
                            nids_flow="flows", gt_bboxes="gt_bboxes", flow_prposals="flow_prposals")
    ...
    def __call__(self, results):
        ...
        idx2key = lambda idx, tostr: str(idx) if tostr else idx
        for im_key in self.im_keys:
            if im_key in results:
                tmp = []
                tostr = isinstance(results[im_key], dict)
                for frame_idx in results['frame_inds']:
                    frame_idx += offset
                    nid = results[im_key][idx2key(frame_idx, tostr)]    # 似乎缓存后都是str key?
                    cur_frame = get_img_from_nid(nid, channel_order='rgb')
                    tmp.append(cur_frame)
                results[self.key_map[im_key]] = tmp
        for flow_key in self.flow_keys:
            # flow_key -> ndarray
            if flow_key in results:
                tmp = []
                tostr = isinstance(results[flow_key], dict)
                for frame_idx in results['frame_inds']:
                    frame_idx += offset
                    nid = results[flow_key][idx2key(frame_idx, tostr)]
                    cur_frame = imdecode(nf.get(nid))       # ndarray
                    tmp.append(cur_frame)
                results[self.key_map[flow_key]] = tmp
        for box_key in self.box_keys:
            if box_key in results:
                tmp = []
                # Assume, if exist box, then its length is equal to len(nori_id_seq).
                tostr = isinstance(results[box_key], dict)
                if len(results[box_key]):
                    for frame_idx in results['frame_inds']:
                        frame_idx += offset
                        cur_box = results[box_key][idx2key(frame_idx, tostr)]
                        tmp.append(cur_box)
                else:
                    tmp = np.zeros((0, 4))
                results[self.key_map[box_key]] = tmp
