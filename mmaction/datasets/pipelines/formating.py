...
def batched_to_tensor(data):
    ...
    if isinstance(data, np.ndarray):
        return torch.from_numpy(data)
    if isinstance(data, Sequence) and not mmcv.is_str(data):
        return [to_tensor(_data) for _data in data]
    if isinstance(data, int):
        return torch.LongTensor([data])
    if isinstance(data, float):
        return torch.FloatTensor([data])
...
class ToTensor:
    ...
    Args:
        ...
        batched (bool): whether to apply element-wise to_tensor for batched data.
    """

    def __init__(self, keys, batched=False):
        self.keys = keys
        self.to_tensor = batched_to_tensor if batched else to_tensor
