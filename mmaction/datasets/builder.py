...
from .samplers import ClassSpecificDistributedSampler, DistributedSampler, SAMPLER
...
def build_dataloader(dataset,
    ...
    # For build from config
    if kwargs.get('forced_sampler', None):
        forced_sampler = kwargs.pop('forced_sampler')
        default_cfg = dict(
            dataset=dataset, num_replicas=world_size, rank=rank,
            shuffle=shuffle, seed=seed)
        sampler = build_from_cfg(forced_sampler, SAMPLER, default_cfg)
        shuffle = False
        batch_size = videos_per_gpu if dist else num_gpus * videos_per_gpu
