...
if not from_mmcv:
    ...
    class SimpleDistEvalHook(EvalHook):  # noqa: F811
        """Distributed evaluation hook.

        This is simpler evaluation hook without the use of dataset.evalute()
        ...
        def __init__(self,
            ...
            self.log_vars = defaultdict(AverageMeter)

        def multi_gpu_test(  # noqa: F811
                self, model, data_loader, tmpdir=None, gpu_collect=True):
            ...
            for data in data_loader:
                with torch.no_grad():
                    outputs = model.train_step(data, optimizer=None)    # forward_train
                    log_vars = outputs['log_vars']
                if rank == 0:
                    # use the first key as main key to calculate the batch size
                    batch_size = outputs.pop('num_samples', len(next(iter(data.values()))))
                    for var_name in log_vars:
                        self.log_vars[var_name].update(log_vars[var_name], batch_size)
        ...
        def _do_evaluate(self, runner):
            ...
            self.multi_gpu_test(
                ...
                tmpdir=tmpdir,
                gpu_collect=self.gpu_collect)
            eval_res = {name: self.log_vars[name].avg for name in self.log_vars.keys()}
            if runner.rank == 0:
                print('\n')
                for name, val in eval_res.items():
                    runner.log_buffer.output[name] = val
