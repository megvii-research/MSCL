...
from mmcv.runner import auto_fp16, _load_checkpoint, load_state_dict
...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    Notes: Compared to BaseRecognizer in official mmaction2, init from ssl pre-training is supported !!!
    ...
    def __init__(self,
        ...
        if backbone['type'].startswith('mmcls.'):
            ...
            self.backbone = torchvision.models.video.__dict__[backbone_type](
                **backbone)
    ...
    def init_weights(self):
        ...
        if self.train_cfg:          # In test, any pretrained will be set to None
            ssl_pretrain = self.train_cfg.get('ssl_pretrain', {})
        elif self.test_cfg:
            ssl_pretrain = self.test_cfg.get('ssl_pretrain', {})
        else:
            ssl_pretrain = {}
        if  ssl_pretrain.get('pretrained', None):
            ssl_state_dict = self.get_ssl_state_dict(ssl_pretrain['pretrained'], logger)
        else:
            ssl_state_dict = None

        if 'backbone' in ssl_pretrain and ssl_state_dict:
            self.init_from_ssl_pretrain('backbone', ssl_state_dict, ssl_pretrain['backbone'], logger)
        ...
        if self.with_cls_head and ssl_state_dict:
            if 'cls_head' in ssl_pretrain:
                self.init_from_ssl_pretrain('cls_head', ssl_state_dict, ssl_pretrain['cls_head'], logger)
            else:
                self.cls_head.init_weights()
        if self.with_neck and ssl_state_dict:
            if 'neck' in ssl_pretrain:
                self.init_from_ssl_pretrain('neck', ssl_state_dict, ssl_pretrain['neck'], logger)
            else:
                self.neck.init_weights()

    def get_ssl_state_dict(self, ssl_cfg, logger):
        filename, map_location = ssl_cfg['filename'], ssl_cfg.get('map_location', None)
        ...
        for p, r in ssl_cfg.get('revise_keys', {}):
            state_dict = {re.sub(p, r, k): v for k, v in state_dict.items()}
        for tar, chosen in ssl_cfg.get('duplicate_keys', {}).items():  # dict
            for k in state_dict.keys():
                if k.startswith(chosen):
                    state_dict[re.sub(chosen, tar, k, count=1)] = state_dict[k]
                    print(f"Exchange {re.sub(chosen, tar, k, count=1)} with {k}")

        return state_dict

    def init_from_ssl_pretrain(self, name, state_dict, ssl_cfg, logger):
        """Initialize the model from self-supervised pretraining weights."""
        structure = getattr(self, name)
        prefix = ssl_cfg['prefix']
        extras = ssl_cfg.get('extras', ["fc"])

        for k in list(state_dict.keys()):
            # retain only encoder_q up to before the embedding layer
            if k.startswith(prefix) and (not any([k.startswith(prefix + "." + ex) for ex in extras])):
                # remove prefix
                state_dict[k[len(prefix) + 1 :]] = state_dict[k]
            # delete renamed or unused k
            del state_dict[k]
        load_state_dict(structure, state_dict, strict=False, logger=logger)
