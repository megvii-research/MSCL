...
@HEADS.register_module()
class TSMHead3D(BaseHead):
    """3D class head for TSM
    ...
    def __init__(self,
        ...
        if self.dropout_ratio != 0:
            ...
            self.dropout = None
        self.fc_cls_list = nn.ModuleList()
        for _ in range(num_clfs):
            self.fc_cls_list.append(nn.Linear(self.in_channels*num_segments//num_clfs, self.num_classes))
    ...
    def init_weights(self):
        """Initiate the parameters from scratch."""
        for fc_cls in self.fc_cls_list:
    ...
    def forward(self, x, num_segs):
        ...
        x = x.view(b, self.num_clfs, -1)     # b, ncf, c
        cls_scores = []
        for i, fc_cls in enumerate(self.fc_cls_list):
            score = fc_cls(x[:, i])
            cls_scores.append(score)
        cls_score = torch.stack(cls_scores, dim=1)  # b, ncf, c

        cls_score = cls_score.mean(1)
        # [N, num_classes]
        return cls_score

    def extract_global_feat(self, x):
        """Defines the computation performed for feature extraction."""
        pass