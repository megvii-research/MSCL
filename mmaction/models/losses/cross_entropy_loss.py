...
@LOSSES.register_module()
class CrossEntropyLoss_torch(torch.nn.modules.CrossEntropyLoss):
    __constants__ = ["ignore_index", "reduction"]
    ignore_index: int

    def __init__(
        self, weight=None, size_average=None, ignore_index: int = -100, reduce=None, reduction: str = "mean",
        loss_weight=1.0,
    ) -> None:
...
@LOSSES.register_module()
class WeightedCrossEntropyLoss_torch(torch.nn.modules.CrossEntropyLoss):
    __constants__ = ["ignore_index", "reduction"]
    ignore_index: int

    def __init__(
        self, weight=None, size_average=None, ignore_index: int = -100, loss_weight=1.0, eps=1e-5
    ) -> None:
        reduce=None
        reduction = "none"
        super().__init__(weight, size_average, ignore_index, reduce, reduction)
    ...
    def forward(self, input, target, pool_weight):
        assert self.weight is None or isinstance(self.weight, torch.Tensor)
        print(pool_weight.shape, "In loss")
        loss = pool_weight*F.cross_entropy(
            input, target, weight=self.weight, ignore_index=self.ignore_index, reduction=self.reduction
        )
        print(loss.shape)
        loss = loss.sum()/(pool_weight.sum() + self.eps)
