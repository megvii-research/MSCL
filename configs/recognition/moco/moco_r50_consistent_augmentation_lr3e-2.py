...
model = dict(
    ...
    mlp=True,
    aux_info=[],
    aug=dict(type="SyncMoCoAugmentV2", crop_size=image_shape[0], sync_level='params', t=num_frames), )
# dataset settings
...
train_pipeline = [
    ...
    dict(
        type="MoCoTransform",
        transform=[dict(type='ToTensorVideo'),],  # n,h,w,c -> c,n,h,w + norm(div255)
...
work_dir = './work_dirs/ssl_train/moco_consistent_aug_lr3e-2'
checkpoint_config = dict(interval=10)
log_config = dict(
    interval=20,
    hooks=[
        dict(type='TextLoggerHook'),
        dict(type='TensorboardLoggerHook', log_dir="/data/mmaction2/tf_logs/ssl_train/moco_consistent_aug_lr3e-2"),
    ])
