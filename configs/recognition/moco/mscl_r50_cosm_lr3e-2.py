...
rgb_recognizer = dict(
    ...
    backbone=dict(
        ...
        out_indices=(0, 1, 2, 3),),
    neck = dict(
        type="TPNMoCo", in_channels=[512, 1024, 2048], out_channels=128,
        sepc_cfg=dict(in_channels=[128, 128, 128], out_channels=128, stride=(1, 2, 2), iBN=False, Pconv_num=1,),
    ),
    moco_head=dict(type="MoCoHead", 
                   basename='', loss_cls=dict(type="CrossEntropyLoss_torch", ignore_index=-1), ),
    im_key="imgs", dim_in=2048, dim=ft_dim,
...
flow_recognizer = dict(
    type='MoCoV2',
    backbone=dict(
        type='resnet_flow.r2d_50',),
    neck = dict(
        type="BaseMoCo",
    ),
    moco_head=dict(type="MoCoHead", basename='flow', 
                   loss_cls=dict(type="CrossEntropyLoss_torch", ignore_index=-1), in_channels=256),
    im_key="imgs", dim_in=256, dim=ft_dim,
    # K=65536, m=0.999, T=0.07, mlp=True, aux_info=[], aug=dict(type="IdentityAug"), )
    K=65536, m_base=0.994, max_iters=dataset_size*total_epochs, T=0.07, mlp=True, aux_info=[], aug=dict(type="IdentityAug"), )
model = dict(
    type="MSCLWithAug",
    ...
    sup_head=dict(type="MSCLWithAugPosHeadV2", basename='', loss_pos=dict(type="CrossEntropyLoss_torch", ignore_index=-1),
                  bkb_channels=(None, 256), t=num_frames//2, T=0.07, 
...
optimizer = dict(
    type='SGD',
    lr=0.0075,  # this lr is used for 4 gpus, in large study, it is set to 0.4 for 512bs
    momentum=0.9,
    weight_decay=1e-4)
...
work_dir = './work_dirs/ssl_train/modist_coaug_samekn_posp_rm_mx_full5_r50_lr2e-2'
checkpoint_config = dict(interval=10)
log_config = dict(
    interval=20,
    hooks=[
        dict(type='TextLoggerHook'),
        dict(type='TensorboardLoggerHook', log_dir="/data/mmaction2/tf_logs/ssl_train/modist_coaug_samekn_posp_rm_mx_full5_r50_lr2e-2"),
    ])
